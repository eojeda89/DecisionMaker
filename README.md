...
- `POST /api/decisions` — toma una decisión para el usuario autenticado (requiere `Authorization: Bearer <token>`). Con `"avoidRepeats": true` (Fase 3.4), reduce —sin eliminar— la chance de que gane la misma opción que ganó recientemente con esta misma lista de opciones: si el resultado coincide con un ganador reciente, se descarta y se vuelve a decidir con probabilidad creciente según cuántas veces ganó (hasta 4 re-tiradas), quedando documentado paso a paso en la respuesta.
